    ...
    const searchFiles = (dir: string) => {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !SKIP_DIRS.includes(entry.name)) {
                ...
                if (
                    lineRegex.test(content) ||
                    blockStartRegex.test(content) ||
                    blockEndRegex.test(content)
                ) {
...
export const generateInstructions = (
    ...
    genertaeAndWriteInstructions(
        ...
        path.join(
            instructionsDir,
            `install-${context.extensionName.toLowerCase().replace(/ /g, '-')}.mdc`
        )
    )
    genertaeAndWriteInstructions(
        ...
        path.join(
            instructionsDir,
            `uninstall-${context.extensionName.toLowerCase().replace(/ /g, '-')}.mdc`
        )
    )
}
 ...
 */
export const genertaeAndWriteInstructions = (
    templateFile: string,
    context: any,
    outputFile: string
) => {
