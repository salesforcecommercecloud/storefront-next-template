...
const createTestFileSystem = (fileContents: any = {}) => {
    ...
    const defaultFiles: Record<string, string> = {
        ...
        [configPath]:
            fileContents.extensionConfig || JSON.stringify(mockedExtensionConfig),
...
describe('create-instructions', () => {
    ...
    const markerValueB = 'SFDC_EXT_featureB'
    ...
    it('getContext throws if extension not found in config', async () => {
        ...
        expect(() => getContext(TEMPLATE_RETAIL_RSC_APP_DIR, 'NOT_FOUND', repo, branch, [], configPath)).toThrow(/not found in extension config/)
    })
    ...
    it('generateInstructions writes install and uninstall files to outputDir', async () => {
        ...
        generateInstructions(TEMPLATE_RETAIL_RSC_APP_DIR, 'SFDC_EXT_featureA', outputRel, 'repo-url', 'main', [], configPath, __dirname + '/templates')
    ...
    it('throws file not found error if file does not exist', async () => {
        const {getContext} = await reloadModule()
        expect(() =>
            getContext(TEMPLATE_RETAIL_RSC_APP_DIR, 'SFDC_EXT_featureA', 'repo-url', 'main', ['doesnotexist'], configPath)
