...
const COMPONENT_SCAN_PATHS: string[] = [
    path.join(SEPARATOR, 'src', SEPARATOR)
]
        ...
        while (i < lines.length) {
            ...
            if (line.includes(SINGLE_LINE_MARKER)) {
                ...
                if (matchingExtension) {
                    ...
                    throw new Error(
                        `Unknown marker found in ${filePath} at line ${i}: \n${line}`
                    )
                }
                    ...
                    try {
                        ...
                        traverse(ast, {
                            ...
                            ImportDeclaration(nodePath: any) {
                                addToRemoveComponentCandidates(nodePath.node.source.value as string)
                            }
                        })
                    } catch (e: unknown) {
                        const error = e as Error
            ...
            if (stats.isDirectory() && !filePath.includes('node_modules')) {
                ...
                traverse(ast, {
                    ...
                    ImportDeclaration(astPath: any) {
                        const importPath = resolvePathFromAlias(astPath.node.source.value as string, projectRoot)
                        if (importPath) {
