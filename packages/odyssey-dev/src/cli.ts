...
program
  .name('odyssey-dev')
  .description('Bundle creation and deployment tool for SFCC Odyssey')
  .version(pkg.version)
...
program
  ...
  .option('-s, --project-slug <slug>', 'Project slug - the unique identifier for your project on Managed Runtime (default: from .env MRT_PROJECT or package.json name)')
...
program
  ...
  .description('Generate LLM instructions using prompt templating for installing and uninstalling Odyssey feature extensions')
  .requiredOption('-d, --project-directory <dir>', 'Project directory')
  ...
  .option('-p, --pwa-repo <repo>', 'PWA repo URL (default: https://github.com/SalesforceCommerceCloud/SFCC-Odyssey.git)')
...
program
  ...
  .requiredOption('-c, --extension-config <config>', 'Extension config JSON file location')
  .option('-e, --extensions <extensions>', 'Comma-separated list of enabled extension marker values (e.g. SFDC_EXT_featureA)')
  .action(async (options) => {
    try {
      // Lazy import to avoid unnecessary load
      const path = await import('path')
