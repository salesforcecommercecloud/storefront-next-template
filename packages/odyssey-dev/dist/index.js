...
const getPwaKitDependencies = (dependencyTree) => {
	...
	const pwaKitDependencies = ["@salesforce/odyssey-dev"];
	const result = {};
...
const createBundle = async (options) => {
	...
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "odyssey-dev-push-"));
...
var CloudAPIClient = class {
	...
	async getHeaders() {
		return {
			"User-Agent": `odyssey-dev@${version}`,
			...this.getAuthHeader()
		};
	}
	/**
...
*/
const buildMrtConfig = (buildDirectory, projectDirectory) => {
	return {
		ssrOnly: [
			"server/**/*",
			"loader.js",
